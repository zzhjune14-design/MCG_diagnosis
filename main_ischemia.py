...
from models.model_st import STNet_MCG  # 使用您的新模型
...
# -----------------------------------------------------------------------------
# 1. 定义单任务训练函数 (只训练 Ischemia)
# -----------------------------------------------------------------------------
def train_epoch_single(model, loader, optimizer, loss_fn, device, label_map, field_target="Ischemia"):
    model.train()
    running_loss = 0.0
    n_samples = 0
    ...
    for Xb, subjects, raws in tqdm(loader, desc="Train Single", leave=False):
        Xb = Xb.to(device)
        ...
        # 兼容性处理：无论模型返回字典还是元组，只取第一个作为 Ischemia
        if isinstance(out, dict):
            logit = out.get(field_target)
        else:
            # 假设模型返回 (logit_isch, logit_xin)，我们只取第一个，用 _ 忽略第二个
            logit, _ = out
        ...
        preds = (probs > 0.5).astype(int).tolist()  # 默认阈值 0.5
...
# -----------------------------------------------------------------------------
@torch.no_grad()
def eval_epoch_single(model, loader, loss_fn, device, label_map, field_target="Ischemia"):
    model.eval()
    running_loss = 0.0
    n_samples = 0
    accum = {'probs': [], 'preds': [], 'trues': []}

    for Xb, subjects, raws in tqdm(loader, desc="Eval Single", leave=False):
        Xb = Xb.to(device)
        y_np = _get_binary_labels_from_raws_or_map(raws, subjects, field_target, label_map)
...
def _metrics_from_acc(a):
    ...
    try:
        if len(np.unique(trues)) > 1:
            ...
            auc_val = 0.5  # 只有一个类别无法计算AUC，给默认值
    except Exception:
        auc_val = 0.0
...
# 3. 主流程函数
# -----------------------------------------------------------------------------
def main_ischemia(pickle_folder, label_csv, out_dir="./output",
                  model_name="STNet_Ischemia_Only",  # 改个名字区分
    ...
    # 数据准备：这里 stratify_by 改为 "Ischemia"，只按缺血分层
    print(f"Building dataloaders (Stratify by {field_isch})...")
    dl_res = build_dataloaders(
        ...
        stratify_by=field_isch  # <--- 关键修改：按缺血分层
    )
    ...
    for fold_idx, (train_loader, val_loader) in enumerate(dataloaders_per_fold):
        ...
        loss_fn = BCEWithLogitsLossWithSmoothing(label_smoothing=0.1)
        ...
        best_val_auc = -1.0  # 单任务通常用 AUC 或 F1 作为最佳标准
        patience_counter = 0
        start_epoch = 1
        ...
        for epoch in range(start_epoch, epochs + 1):
            # --- Train ---
            t_loss, t_met, _ = train_epoch_single(model, train_loader, optimizer, loss_fn, device, label_map,
                                                  field_isch)

            # --- Eval ---
            v_loss, v_met, v_accum = eval_epoch_single(model, val_loader, loss_fn, device, label_map, field_isch)
            ...
            print(
                f"  [Val  ] Acc: {v_met['acc']:.4f}  F1: {v_met['f1']:.4f}  AUC: {v_met['auc']:.4f}  Sens: {v_met['sens']:.4f}  Spec: {v_met['spec']:.4f}")
            print("-" * 60)
            ...
            current_metric = v_met['auc']  # 以 AUC 为标准
        ...
        # Fold 结束：保存曲线图
        plot_metrics_curves(plots_dir, f"Ischemia_Fold{fold_idx}",
...
if __name__ == "__main__":
    ...
    main_ischemia(
        ...
        out_dir=cfg['out_dir'],
        model_name="STNet_Ischemia_Only",  # 独立目录，互不影响
        batch_size=32,  # 推荐 32
        epochs=70,  # 推荐 70
        patience=15  # 早停
    )